...
def train(env, sb3_algo):
    match sb3_algo:
        case "A2C":
            ...
            model = A2C(
                "MlpPolicy", env, device=device, verbose=1, tensorboard_log=log_dir
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train or test a model using stable baselines3"
    )
    parser.add_argument(
        "gymenv",
        type=str,
        help="The gym environment to train/test the model on i.e. Humanoid-v4",
    )
    parser.add_argument(
        "sb3_algo",
        type=str,
        help="The algorithm to use for training/testing the model i.e. A2C, SAC, TD3",
    )
    parser.add_argument("-t", "--train", action="store_true", help="Train the model")
    parser.add_argument(
        "-s", "--test", metavar="path_to_model", help="Path to the model to test"
    )
    args = parser.parse_args()

    if args.train:
        print(f"Training {args.sb3_algo} on {args}")
